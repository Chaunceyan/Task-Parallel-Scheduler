...
find_optimal(optimal_solution(BestSol, ET)):-
	...
	generate_dependency_solution(SolutionList),
...
generate_dependency_solution(solution(Solution)):-
	...
	generate_basic_solution(AllCores, Schedules),
	form_schedules(Schedules, Paths, Solution).
	...
	form_schedules([schedule(Core, Tasks)|Schedules], [Path|Paths], Solution):-
		append(Tasks, Path, NewTasks),
		append([schedule(Core, NewTasks)], [], SubSchedules),
		form_schedules(Schedules, Paths, SubSchedules, Solution).
	form_schedules([], [], SubSchedules, Solution):-
		append(SubSchedules, [], Solution).
	form_schedules([schedule(Core, Tasks)|Schedules], [], SubSchedules, Solution):-
		append(Schedules, SubSchedules, Solution).
	form_schedules([], [Path|Paths], SubSchedules, Solution):-
		form_schedules(SubSchedules, Paths, [], Solution).
	form_schedules([schedule(Core, Tasks)|Schedules], [Path|Paths], SubSchedules, Solution):-
		append(Tasks, Path, NewTasks),
		append(SubSchedules, [schedule(Core, NewTasks)], NewSubSchedules),
		form_schedules(Schedules, Paths, NewSubSchedules, Solution).
	...
	findall_critical_path(AllTasks, Paths):-
		...
		find_critical_path([Task], AllTasks, [], CriticalPaths),
	
		lfsort(CriticalPaths, SortedCriticalPaths),
		my_last(CriticalPath, SortedCriticalPaths),
	...
	find_critical_path([], ListAvailable, CriticalPath, CriticalPaths):-
		append([], [], CriticalPaths).
	find_critical_path([Task|Tasks], ListAvailable, CriticalPath, CriticalPaths):-
		(member(Task, ListAvailable),!,
		findall(SubTask, depends_on(SubTask, Task, _), SubTasks),
		(length(SubTasks, 0),!,
		append(CriticalPath, [Task], OneCriticalPath),
		append([OneCriticalPath], [], CriticalPaths);
		append(CriticalPath, [Task], NewCriticalPath),
		find_critical_path(Tasks, ListAvailable, CriticalPath, OtherPaths),
		find_critical_path(SubTasks, ListAvailable, NewCriticalPath, Paths),
		append(Paths, OtherPaths, CriticalPaths));
		append([CriticalPath], [], CriticalPaths)).
